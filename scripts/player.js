class Player {
  ...
  prettyTime(timeInSeconds) {
    const prettyTimeMinutes = Math.floor(timeInSeconds / 60);
    const prettyTimeSeconds = Math.floor(
      timeInSeconds - prettyTimeMinutes * 60
    );
    return `${prettyTimeMinutes}:${
      prettyTimeSeconds > 9
        ? prettyTimeSeconds
        : "0" + String(prettyTimeSeconds)
    }`;
  }
}
